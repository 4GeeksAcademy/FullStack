...
const CartasBellezaOcho = ({ onViewService = () => {} }) => {
  ...
  useEffect(() => {
    const fetchBellezaDeals = async () => {
      try {
        ...
        // Realizamos la solicitud fetch para obtener las ofertas de belleza
        const response = await fetch(`${backendUrl}/belleza`);
        ...
        console.log("Respuesta de la API de Belleza: ", data);
        if (!Array.isArray(data.belleza)) {
          throw new Error("Los datos recibidos no son una lista.");
        }

        setDeals(data.belleza); // Guardamos las ofertas en el estado
      } catch (err) {
        console.error("Error al cargar las ofertas de belleza:", err);
        setError("Hubo un error al cargar las ofertas de belleza.");
      } finally {
